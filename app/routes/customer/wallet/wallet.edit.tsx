   ...
   if (request.method === "PATCH") {
      try {
         if (file && file instanceof File && file.size > 0) {
            ...
            const folderPath = await resolveUserUploadPath("customer", customerId, "slip");
            const buffer = Buffer.from(await file.arrayBuffer());
            const url = await uploadFileToBunnyServer(buffer, file.name, file.type, folderPath, "slip");
         ...
         if (res.id) {
            ...
            if (originPaymentSlip && file && file instanceof File && file.size > 0) {
               await deleteFileFromBunny(extractFilenameFromCDNSafe(originPaymentSlip as string)).catch((err) =>
