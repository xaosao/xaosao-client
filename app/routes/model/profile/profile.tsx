...
export async function loader({ request }: LoaderFunctionArgs) {
    ...
    return { model, banks, profilePosts, VITE_FRONTEND_URL: import.meta.env.VITE_FRONTEND_URL };
}
...
export default function ModelProfilePage() {
    ...
    const { model, banks, profilePosts, VITE_FRONTEND_URL } = useLoaderData<{ model: IModelOwnProfileResponse; banks: IModelBank[]; profilePosts: any[]; VITE_FRONTEND_URL: string }>();
    ...
    const shareProfileUrl = `${VITE_FRONTEND_URL}dashboard/user-profile/${model.id}`;
    ...
    const copyShareLink = async () => {
        try {
            if (navigator.clipboard && window.isSecureContext) {
                ...
                const success = fallbackCopyToClipboard(shareProfileUrl);
    ...
    const handleSocialShare = (option: typeof socialShareOptions[0]) => {
        const text = t('modelProfile.shareText', {
            name: model.firstName,
            defaultValue: `Check out ${model.firstName}'s profile on XaoSao!`
        });
        const shareUrl = option.getShareUrl(shareProfileUrl, text);
                        ...
                        <div className="hidden sm:block flex-1">
                            ...
                            <div className="flex items-center gap-3 mb-6">
                                ...
                                <Button
                                    ...
                                    onClick={() => setShareModalOpen(true)}
                                >
                                    <Share2 /> {t("modelProfile.share", "Share")}
                                </Button>
                            </div>
                        </div>
                    </div>
